...
static node_t *
node_alloc(int num_levels, map_key_t key, map_val_t val)
{
...
static node_t *
find_preds(node_t **preds, node_t **succs, int n, skiplist_t *sl, map_key_t key, enum unlink unlink)
{
  node_t *pred = sl->head;
  node_t *item = NULL;
  ...
  for (level = sl->high_water - 1; level >= 0; --level) {
...
map_val_t
sl_lookup(skiplist_t *sl, map_key_t key)
{
...
static map_val_t
update_item(node_t *item, map_val_t expectation, map_val_t new_val)
{
...
map_val_t
sl_cas(skiplist_t *sl, map_key_t key, map_val_t expectation, map_val_t new_val)
{
  ...
  int n = random_levels(sl), level, i;
  ...
  if (EXPECT_FALSE(expectation != (map_val_t)CAS_EXPECT_DOES_NOT_EXIST &&
                   expectation != (map_val_t)CAS_EXPECT_WHATEVER)) {
  ...
  for (level = 1; level < new_item->num_levels; ++level) {
  ...
  for (level = 1; level < new_item->num_levels; ++level) {
	...
	break; // successfully linked <new_item> into the skiplist at the current <level>
      ...
      for (i = level; i < new_item->num_levels; ++i) {
...
map_val_t
sl_remove(skiplist_t *sl, map_key_t key)
{
  int level;
  ...
  for (level = item->num_levels - 1; level >= 0; --level) {
    markable_t next;
...
void
sl_print(skiplist_t *sl, int verbose)
{
  ...
  if (verbose) {
	...
	printf("[HEAD]");
      ...
      for (level = 1; level < item->num_levels; ++level) {
...
map_val_t
sl_iter_next(sl_iter_t *iter, map_key_t *key_ptr)
{
  ASSERT(iter);
